...
Chaque voiture est identifiée par son numéro d'immatriculation.  Elle est également caractérisée par sa marque, son modèle, sa couleur et son prix et une date de fabrication.
Une voiture peut avoir été achetée; auquel cas, elle a un propriétaire, et une date d'achat.
### Etape 1 : créez le schéma logique de la table
Avant de commencer, il est importer de créer le schéma logique de la table (vous savez, la notation graphique dans une petite boîte qui indique le nom de la table, et liste les attributs avec le type associé, et les contraintes éventuelles).
### Etape 2 : créez le schéma physique de la table
